...
@Component({
  selector: 'app-irrigationmaster',
  templateUrl: './irrigationmaster.component.html',
  styleUrls: ['./irrigationmaster.component.scss']
})
export class IrrigationmasterComponent implements OnInit {
  ...
  fetchData() {
    ...
    this.backendService.fetchRecords(data).subscribe((res) => {
      if (res.status === true) {
        this.irrigationRecords = res.result;
        this.url = res.url;
        this.rerender();
        this.spinner.hide();
      } else {
